...
async def mqtt_background_task():
    ...
    try:
        ...
        async with aiomqtt.Client(hostname=host, port=1883, username=usr, password=pwd) as client:
            ...
            logger.info("Bot conectado a Mosquitto (Routing Interno)")
            async for message in client.messages:
                ...
                if tema == "28:cd:c1:05:57:12":
                    try:
                        ...
                        logger.info(f"Caché actualizada: Temp {estado_termostato['temperatura']}°C")
                    except json.JSONDecodeError:

                        logger.error("Recibido payload MQTT no válido.")
...
async def manejar_teclado_principal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if texto == "Estado Actual":
        msg = (
            ...
            f"*Relé:* {'PRENDIDO 🟢' if estado_termostato['rele'] == 1 else 'APAGADO 🔴'}\n"
            f"*Periodo de envío:* {estado_termostato['periodo']} seg"
        )
        ...
        await update.message.reply_text("Seleccioná el modo de operación:", reply_markup=reply_markup)

    elif texto == "Control Relé":
        if estado_termostato["modo"] == "automatico":
            await update.message.reply_text("El termostato está en modo *Automático*. Cambialo a Manual para usar esta función.", parse_mode='Markdown')
        else:
            teclado_inline = [
            ...
            reply_markup = InlineKeyboardMarkup(teclado_inline)
            await update.message.reply_text("Control manual del relé:", reply_markup=reply_markup)
        ...
        if mqtt_client_global:
            ...
            await update.message.reply_text("Error: No hay conexión con el Broker MQTT.")
...
async def manejar_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dato.startswith("modo_"):
        nuevo_modo = dato.split("_")[1] # extrae 'automatico' o 'manual'
        if mqtt_client_global:
        ...
        estado_rele = dato.split("_")[1] # extrae '1' o '0'
        ...
        if mqtt_client_global:
            ...
            await query.edit_message_text(f"Orden enviada: {accion} relé.")

async def pedir_setpoint(update: Update, context: ContextTypes.DEFAULT_TYPE):

    if update.message.text == "Setpoint":
        await update.message.reply_text("Por favor, escribí el nuevo valor de Setpoint (ejemplo: 24.5) o /cancelar para salir:")
...
async def guardar_setpoint(update: Update, context: ContextTypes.DEFAULT_TYPE):
    texto = update.message.text
    try:
        nuevo_setpoint = float(texto.replace(",", ".")) # Por si ponen coma en vez de punto
        if nuevo_setpoint >= 0:
            if mqtt_client_global:
            ...
            await update.message.reply_text("El setpoint no puede ser negativo. Intentá con otro número:")
            return ESPERANDO_SETPOINT

    except ValueError:
        await update.message.reply_text("Formato inválido. Ingresá un número (ej: 25.5):")
        return ESPERANDO_SETPOINT
